...
# - a = pôle ACCÉLÉRATEUR = Transhumanisme (Transhumanism), b = pôle RALENTISSEUR = Humanisme
#   (Humanism) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Transhumanisme (accélérateur)
    ...
    # cartes) : arguments_a = la slide de Transhumanisme, arguments_b = celle de
    # Humanisme. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
    ...
    "arguments_a": {                # Transhumanisme (accélérateur)
    ...
    "arguments_b": {                # Humanisme (ralentisseur)
        "zoom": None,                  # auto : min(240, palier 130/120/110 selon le titre le plus long) ; un absolu passe outre le plafond
        "zoom_scale": 0.9,
        "badge_scale": None,           # facteur du badge de nature (None = taille du DS)
    },
