...
# - a = pôle ACCÉLÉRATEUR = Rationalité (Rationality), b = pôle RALENTISSEUR = Émotion
#   (Emotion) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Rationalité (accélérateur)
    ...
    "identity_b": {                 # Émotion (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
        "statement_zoom_scale": 0.95,  # ex. 1.15 = calcul auto ×1,15
        "mascot_vh": 27.0,             # hauteur des 2 mascottes (vh)
        "mascot_vh_scale": None,
    ...
    # cartes) : arguments_a = la slide de Rationalité, arguments_b = celle de
    # Émotion. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
    ...
    "arguments_a": {                # Rationalité (accélérateur)
    ...
    "arguments_b": {                # Émotion (ralentisseur)
        "zoom": None,                  # auto : min(240, palier 130/120/110 selon le titre le plus long) ; un absolu passe outre le plafond
        "zoom_scale": 0.85,
        "badge_scale": None,           # facteur du badge de nature (None = taille du DS)
    },
