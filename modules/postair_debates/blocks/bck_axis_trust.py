...
# - a = pôle ACCÉLÉRATEUR = Confiance (Trust), b = pôle RALENTISSEUR = Autonomie de jugement
#   (Self-reliance) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Confiance (accélérateur)
    ...
    "identity_b": {                 # Autonomie de jugement (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
        "statement_zoom_scale": None,  # ex. 1.15 = calcul auto ×1,15
        "mascot_vh": 27.0,             # hauteur des 2 mascottes (vh)
        "mascot_vh_scale": None,
    ...
    # cartes) : arguments_a = la slide de Confiance, arguments_b = celle de
    # Autonomie de jugement. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
    ...
    "arguments_a": {                # Confiance (accélérateur)
    ...
    "arguments_b": {                # Autonomie de jugement (ralentisseur)
        "zoom": None,                  # auto : min(240, palier 130/120/110 selon le titre le plus long) ; un absolu passe outre le plafond
        "zoom_scale": 0.9,
        "badge_scale": None,           # facteur du badge de nature (None = taille du DS)
    },
