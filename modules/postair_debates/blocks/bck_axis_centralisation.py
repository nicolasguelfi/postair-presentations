...
# - a = pôle ACCÉLÉRATEUR = Centralisation (Centralisation), b = pôle RALENTISSEUR = Décentralisation
#   (Decentralisation) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Centralisation (accélérateur)
    ...
    "identity_b": {                 # Décentralisation (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
        "statement_zoom_scale": None,  # ex. 1.15 = calcul auto ×1,15
        "mascot_vh": 27.0,             # hauteur des 2 mascottes (vh)
        "mascot_vh_scale": None,
    ...
    # cartes) : arguments_a = la slide de Centralisation, arguments_b = celle de
    # Décentralisation. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
    ...
    "arguments_a": {                # Centralisation (accélérateur)
    ...
    "arguments_b": {                # Décentralisation (ralentisseur)
        "zoom": None,                  # auto : min(240, palier 130/120/110 selon le titre le plus long) ; un absolu passe outre le plafond
        "zoom_scale": None,
        "badge_scale": None,           # facteur du badge de nature (None = taille du DS)
    },
