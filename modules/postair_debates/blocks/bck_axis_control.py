...
# - a = pôle ACCÉLÉRATEUR = Liberté (Freedom), b = pôle RALENTISSEUR = Contrôle
#   (Control) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Liberté (accélérateur)
    ...
    "identity_b": {                 # Contrôle (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
        "statement_zoom_scale": None,  # ex. 1.15 = calcul auto ×1,15
        "mascot_vh": 27.0,             # hauteur des 2 mascottes (vh)
        "mascot_vh_scale": None,
    ...
    # cartes) : arguments_a = la slide de Liberté, arguments_b = celle de
    # Contrôle. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
