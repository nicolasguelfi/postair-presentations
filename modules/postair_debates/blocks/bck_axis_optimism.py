...
# - a = pôle ACCÉLÉRATEUR = Optimisme (Optimism), b = pôle RALENTISSEUR = Pessimisme
#   (Pessimism) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Optimisme (accélérateur)
    ...
    "identity_b": {                 # Pessimisme (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
    ...
    # cartes) : arguments_a = la slide de Optimisme, arguments_b = celle de
    # Pessimisme. Sur une slide, les 3 cartes partagent UN SEUL zoom (règle NG
    ...
    "arguments_a": {                # Optimisme (accélérateur)
    ...
    "arguments_b": {                # Pessimisme (ralentisseur)
        "zoom": None,                  # auto : min(240, palier 130/120/110 selon le titre le plus long) ; un absolu passe outre le plafond
        "zoom_scale": 0.85,
        "badge_scale": None,           # facteur du badge de nature (None = taille du DS)
    },
