...
# - a = pôle ACCÉLÉRATEUR = Individualisme (Individualism), b = pôle RALENTISSEUR = Altruisme
#   (Altruism) ; waves_* ne se rend que si le
...
TUNING: dict = {
    ...
    "identity_a": {                 # Individualisme (accélérateur)
    ...
    "identity_b": {                 # Altruisme (ralentisseur)
        "statement_zoom": None,        # auto : 116 (≤6 lignes) / 106 (≤8) / 98
        "statement_zoom_scale": None,  # ex. 1.15 = calcul auto ×1,15
        "mascot_vh": 27.0,             # hauteur des 2 mascottes (vh)
        "mascot_vh_scale": None,
...
def build(lang: str = "en", **_):
    axis_slides("altruism", lang=lang, tuning=TUNING)
