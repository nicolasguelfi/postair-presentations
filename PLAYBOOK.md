...
| Rythme du débat par axe : identité du pôle A, identité du pôle B, SCÈNE DU DÉBAT (3 colonnes — mascottes + énoncé synthétique `pole.synthesis` de chaque pôle, Voxo au micro à 70 % de sa taille d'intro, sous-titre « hands on — mic — tell us »), puis le matériau pôle par pôle (vagues, figures, arguments) — la salle débat AVANT de voir le matériau ; face-à-face de fin d'axe et slide micro d'intro retirés (réversibles : fonction et bloc conservés, commentés) ; titre court quand l'axe est homonyme d'un pôle | 2026-08-31 | `debates/custom/render.py::_debate_stage`, `axis_slides`, `debates/book.py` |
| Briques d'axe PARAMÉTRÉES : chaque sous-slide accepte ses réglages de taille (absolu `statement_zoom=120` OU facteur `statement_zoom_scale=1.15`, défaut = calcul auto) ; la main de l'artiste vit dans `TUNING = {}` de chaque `bck_axis_*` (clés `identity_a/b`, `stage`, `waves_a/b`, `figure_a1..b3`, `arguments_a/b` — clé inconnue = erreur bruyante). PAS de filet outillé (décision NG) : après toute campagne hub, repasser VISUELLEMENT sur les axes réglés — un réglage absolu fige le rendu du texte d'aujourd'hui | 2026-08-31 | `debates/custom/render.py::_tuned`, docstring d'`axis_slides` |
| Pôle SANS CHAMPION (audit hub 260831, rapport `_reports/audit-champions-260831.md` du hub) : la sélection des figures ne franchit JAMAIS la médiane de l'axe (MEDIAN_DISTANCE=50) et la relaxation COMPLÈTE les retenus ≤ 40 au lieu de repartir de zéro (bug qui évinçait Marinetti/INDI au profit de trois altruistes à 73-80) ; moins d'un trio du bon côté → gel `no_champion: true`, `figures: []`, les 3 plus proches (`closest`) au tooltip, et la slide « Before us — no one? » assume l'absence (décision A NG). Arbitrage NG en suspens : Marinetti seul champion INDI possible (30 pts) mais déjà sur OPTI + controverse — no_champion retenu | 2026-08-31 | `build_debates_content.py` (sélection, gel v2, `corpus_figures`), `render.py::_absence` |
| AMENDEMENT banc partiel (NG 2026-08-31 soir) : « trio de préférence, sinon deux, sinon une seule figure » — `no_champion` (slide d'absence) ne vaut plus que pour ZÉRO figure éligible ; 1-2 figures = banc partiel, avertissement au gel, AUCUN bandeau à l'écran (Q2b — l'asymétrie EST l'information) ; conséquence : Marinetti champion unique d'Individualisme (30 pts, citation « Beauty exists only in struggle » arbitrée par debate_choice hub, FR = original du Figaro 1909, clé cited) | 2026-08-31 | `build_debates_content.py` (sélection), `render.py::_figure` (bandeau retiré) |
| Piste Edison × Individualisme CLOSE (négatif, rapport hub `_reports/edison-indi-260831.md`) : « Anything that won't sell, I don't want to invent » reste `reported` — attestation de Young 1890 impossible (livre non numérisé, Wikiquote a retiré la citation), aucune autre citation verbatim ALT côté individualiste (Rosanoff 1932 = souvenir posthume, inéligible) ; banc INDI assumé à 1/3 (Marinetti seul). Réouverture SI une numérisation de Young 1890 apparaît | 2026-08-31 | rapport hub ; règle d'éligibilité `grade == verbatim` de `build_debates_content.py` |
| Piste médias Gandhi + Hawking DIFFÉRÉE (NG 2026-08-31) : représentations PROTÉGÉES et sensibilité élevée — Gandhi : nom et représentation picturale protégés en Inde (Emblems and Names Act, 1950, autorisation du gouvernement central requise) ; Hawking : succession active sur le nom/l'image (marque sonore déposée sur sa voix de synthèse — l'objet même qu'un portrait parlant IA imiterait). En outre, la simulation montre que leurs médias re-castent 8 trios sur 18 (domino des plafonds de réutilisation : Érasme et Benjamin sortiraient) + 2 dettes de référence nouvelles (Galilée/OPEN, Shen Kuo/TRAN). Réouverture éventuelle : après l'AI Day, avec un dossier de clearance spécifique arbitré par NG — jamais par simple production de médias | 2026-08-31 | simulation build() avec assets forcés ; `media-manifest.json` (les deux absents) |
| Limites de l'audit CONSIGNÉES (NG Q2a 2026-08-31) : LeCun/Liberté à la borne exacte du seuil 40 (citation SB1047 franchement pro-pôle, RAS) ; Trithème/Humanisme : ligne TRA-02 low-confidence à contre-sens au tooltip — le gabarit l'affiche déjà avec la mention « points AWAY » prévue pour ce cas, aucun geste ; Ibn Khaldun/Centralisation : citation descriptive remplacée par debate_choice hub vers Muqaddima V §18 « the dynasty is the biggest market » (prompt du 2026-08-31) | 2026-08-31 | audit hub §7 ; `render.py::_why_here` (mention points AWAY) |
