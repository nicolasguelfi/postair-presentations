...
## Détail git : ignoré ET versionné, les deux exprès
...
- Le lien lui-même est **versionné** (ajouté en force, l'ignore ne détrackant
  pas un fichier suivi) : un clone git nu du dépôt reçoit donc aussi le lien,
  même hors Dropbox.
...
# 4. git : ignorer les .venv réels (sans barre finale — couvre lien ET
#    dossier), puis versionner le lien malgré l'ignore
printf '%s\n' '.venv' >> .gitignore
git add .gitignore
git add -f .venv
